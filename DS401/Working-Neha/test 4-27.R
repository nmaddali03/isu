...
# Filter data for the "hit and run" from other category
category_data <- cfs_filtered %>%
  filter(cfs_type == "Hit and run collision")

# Aggregate the data: predicting the number of calls in each hour
hourly_calls <- category_data %>%
  group_by(cfs_year, cfs_month, cfs_day, cfs_hour) %>%
...
set.seed(123)  # For reproducibility
rf_model <- randomForest(total_calls ~ cfs_year + cfs_month + cfs_hour + cfs_day, data = train_data)
rf_model
...
rf_cv <- train(total_calls ~ cfs_year + cfs_month + cfs_hour + cfs_day,
               data = train_data,
               method = "rf",
               tuneGrid = grid,
...
ntree <- 500
rf_model_tuned <- randomForest(total_calls ~ cfs_year + cfs_month + cfs_hour + cfs_day,
                               data = train_data,
                               mtry = best_mtry,
                               ntree = ntree)
rf_model_tuned
...
set.seed(123)  # For reproducibility
bagged_rf_model <- randomForest(total_calls ~ cfs_year + cfs_month + cfs_hour + cfs_day,
                                data = train_data,
                                mtry = best_mtry,
                                ntree = ntree,
                                importance = TRUE,
...
# Filter data for the "hit and run" from other category
category_data <- cfs_filtered %>%
  filter(cfs_type == "Theft or Shoplifting")

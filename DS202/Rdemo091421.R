...
lvl3 <- c('Larcency.theft', 'Burglary', 'Motor.vehicle.theft',
...
type5 <- factor(fbi$Type, levels = lvl3,
                labels=c('theft','burglary','v.theft','assault',
                         'robbery','l.rape','murder','rape'))
...
type9 <- factor(fbi$Type, lables =c('assault', 'burglary',
                                    'theft',
                                    'l.rape',
                                    'v.theft',
                                    'murder',
...
type10 <- reorder(type9, fbi$Rate, FUN = median, ra.rm=TRUE)

...
void io_poke() {
    int rand_poke, rand_move, i, man_distance, level, moves_start, moves_end, move_one_id, move_two_id, stat_count, j;
    ...
    if(pokes[rand_poke].species_id < 808) {
        ...
        for(i = 0; i < (int)pokemon_moves_20_vec.size(); i++) {
            if(pokemon_moves_20_vec[i].pokemon_id == pokes[rand_poke].species_id) {
                moves_start = i;
                break;
            }
        }
        for(i = moves_start; i < (int)pokemon_moves_20_vec.size(); i++) {
            if(pokemon_moves_20_vec[i].pokemon_id != pokes[rand_poke].species_id) {
                moves_end = i;
                break;
            }
            else if(i == (int)pokemon_moves_20_vec.size() - 1) {
                moves_end = (int)pokemon_moves_20_vec.size();
                break;
            }
        }
        
        for(i = moves_start; i < moves_end; i++) {
            if(pokemon_moves_20_vec[i].pokemon_move_method_id == 1 && pokemon_moves_20_vec[i].level <= level && pokemon_moves_20_vec[i].level != 0) {
                possible_moves.push_back(pokemon_moves_20_vec[i].move_id);
            }
        }
    }
    ...
    mvprintw(3, 16, "A wild Pokemon has appeared!");
    mvprintw(5, 16, "Name: %s", pokes[rand_poke].name.c_str());
    mvprintw(6, 16, "Level: %d", level);
    mvprintw(7, 16, "Gender: %s", gender.c_str());
    mvprintw(8, 16, "Shiny?: %s", shiny.c_str());
    mvprintw(9, 16, "Move One: %s", move_one.c_str());
    mvprintw(10, 16, "Move Two: %s", move_two.c_str());
    mvprintw(12, 16, "HP: %d, IV's: %d", stats[0], ivs[0]);
    mvprintw(13, 16, "Attack: %d, IV's: %d", stats[1], ivs[1]);
    mvprintw(14, 16, "Defense: %d, IV's: %d", stats[2], ivs[2]);
    mvprintw(15, 16, "Special Attack: %d, IV's: %d", stats[3], ivs[3]);
    mvprintw(16, 16, "Special Defense: %d, IV's: %d", stats[4], ivs[4]);
    mvprintw(17, 16, "Speed: %d, IV's: %d", stats[5], ivs[5]);
    mvprintw(19, 16, "Press any key to run!");
    getch();
}

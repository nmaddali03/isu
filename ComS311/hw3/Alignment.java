 ...
 * @author: Neha Maddali
 * 
 * A class to implement a dynamic programming algorithm for
 * computing the edit distance between two sequences and
 * their alignment with this mimimum distance.
 *
 * The algorithm computes the cost matrix in a reverse order
 * so that the optimal alignment can be recovered in a forward order,
 * with its first pair reported and saved at the beginning of each array.
 *
 * Acknowledgements
 *
 * This template is created by Xiaoqiu Huang.
 * This project was conceived during discussions with the other instructors
 * (David Fernandez-Baca and Seyedehzahra Hosseini) for Com S 311 in Spring 2022.
 */
...
public class Alignment
{
	...
	/*
Input parameter seqone is a reference to a Sequence object
containing the name, length and character array of one sequence.
Input parameter seqtwo is for another sequence.
Input parameter param is a reference to a SubstitutionGap object
containing a match score, a mismatch penalty and a gap penalty.
If seqone is null, throws
an RuntimeException exception with the message "Sequence object one is null."
If seqtwo is null, throws
an RuntimeException exception with the message "Sequence object two is null."
If param is null, throws
an RuntimeException exception with the message "SubstitutionGap object is null."

This method computes the cost matrix in a reverse order and
produces the alignment by doing traceback in a forward order, with
the first alignment pair saved at the beginning of
the three character arrays (top, mid and bot).

If at most one of the two sequences is empty (of length 0), the method is expected 
to work correctly by producing an alignment made of only deletions or insertions.
However, if both sequences are of length 0, then the method throws
an RuntimeException exception with the message "Both sequences are empty."

The method obtains one sequence and its length from the first parameter seqone, and
saves them in the char[] array A and the int scalar variable M,
with the first letter of the sequence stored at index 1 of A.
Similarly, the other sequence and its length from the second parameter seqtwo
are saved in the char[] array B and the int scalar variable N,
with the first letter of the sequence stored at index 1 of B.

Next the method obtains a match score, a mismatch penalty, and an insertion/deletion penalty
from the third parameter param. If letters a and b are identical, then
sigma(a, b) is the match score; otherwise, sigma(a, b) is the mismatch penalty. 
The insertion/deletion penalty is denoted by r in the assignment specification.

The method computes the cost matrix S(i, j) for 0 <= i <= M and 0 <= j <= N,
according to the algorithm in the assignment specification, where
M and N are the sequence lengths m and n in the assignment specification, and
A[i] contains the letter a_i and B[j] contains the letter b_j in the specification.
The matrix S is implemented by using the int [][] array variable Smat of
M+1 rows and N+1 columns. After the matrix S is computed, the method saves S(0, 0),
the edit distance, in the int scalar variable editDistance.

Next the method generates an optimal sequence alignment by using
the traceback algorithm in the assignment specification.
To save the alignment, the three char[] arrays top, mid and bot
of sufficient length, say, M+N+2, are created.
Let (x_1, y_1) (x_2, y_2) ... (x_k, y_k) be all pairs of
the alignment of length k in left-to-right order.
Then, for 1 <= j <= k,
top[j-1] is set to x_j, bot[j-1] is set to y_j, and
mid[j-1] is set to '|' if x_j == y_j and ' ' (a single space) otherwise.
Each of the remaining slots in each array is set to ' '.
The alignment length variable alignlen is set to k.
The variable editnum, which denotes the number of differences in the alignment,
is set to the number of pairs (x_j, y_j) such that x_j != y_j.
	...
	public Alignment(Sequence seqone, Sequence seqtwo, SubstitutionGap param)
	{
		...
		// to 0 and S(m, n) is the edit distance. This requirement is enforced in this template.
		...
		M = seqone.getLength(); //sequence of length m
		A = seqone.getSeq(); //sequence in array
		N = seqtwo.getLength(); //sequence of length n
		B = seqtwo.getSeq(); //sequence in array
		int r = param.getgap(); //penalty score for each letter in a gap

		//all three of the following ArrayLists make up OA
		Smat = new int[M+1][N+1]; //matrix S implemented empty 2d array 
		Smat[M][N] = 0; 
		int sigma;

		//following pseudo code from assignment spec figure 1
		for(int j = N-1; j >= 0; j--) {
		...
		for(int i = M-1; i >= 0; i--) {
			...
			for(int j = N-1; j>=0; j--) {
				sigma = param.getmatch(); //letters a and b are identical so match score
				if(A[i+1] != B[j+1]) { //letters a and b are not identical so mismatch penalty
					sigma = param.getmismat();
				}
				//calculate the values of matrix S given as in pseudo code
				Smat[i][j] = Math.min(Math.min(Smat[i+1][j+1] + sigma, Smat[i+1][j] + r), Smat[i][j+1]+r);
			}
		}

		ArrayList<Character> listTop = new ArrayList<Character>();
		ArrayList<Character> listMid = new ArrayList<Character>();
		ArrayList<Character> listBot = new ArrayList<Character>();
		int i = 0; int j = 0; //following figure 2 pseudo code of assignment spec, i and j = 0
		while(i <= M && j <= N) {
			if(i == M && j == N) {
				break;
			}
		...
		//report pairs of the alignment in OA -- first get the size of each arrays
		alignlen = listTop.size();
		top = new char[listTop.size()];
		bot = new char[listBot.size()];
		mid = new char[listMid.size()];

		for(int z = 0; z < top.length; z++) {
			top[z] = listTop.get(z);
			mid[z] = listMid.get(z);
			bot[z] = listBot.get(z);
		}

		//get the editDistance which is the 0,0 position of the S matrix
		//for (int c = 0; c<1; c++) {
			//editDistance = Smat[c][0];
		//}
